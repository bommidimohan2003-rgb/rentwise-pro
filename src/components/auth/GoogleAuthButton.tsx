...
export function GoogleAuthButton({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      console.warn("[Google Auth] Firebase Google Popup error:", err);
      // If Firebase API key is demo placeholder or popup domain is unconfigured, prompt Gmail modal fallback
          ...
          <div className="bg-card border border-border rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 relative">
            ...
            <form onSubmit={handleGmailSubmit} className="space-y-4 text-left">
              ...
              <div className="p-3 rounded-xl bg-secondary/50 border border-border text-[11px] text-muted-foreground flex gap-2 items-start">
                ...
                <span>
                  Tip: To enable native 1-click Google Popup, replace `VITE_FIREBASE_API_KEY` in `.env` with your real key from Firebase Console.
                </span>
              </div>
