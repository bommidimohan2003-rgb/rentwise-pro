        ...
        <div className="grid lg:grid-cols-2 gap-10">
          <div>
            <div className="aspect-[4/3] rounded-2xl overflow-hidden bg-secondary">
              <img src={gallery[activeImg]} alt={product.title} className="h-full w-full object-cover" />
            </div>
            <div className="mt-4 grid grid-cols-4 gap-3">
              {gallery.map((g, i) => (
                <button
                  ...
                  className={cn("aspect-square rounded-xl overflow-hidden border-2 transition-all", activeImg === i ? "border-primary" : "border-transparent opacity-70")}
                >
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => toggle(product.id)} className="rounded-full p-2 border border-border">
                <Heart className={cn("h-4 w-4", has(product.id) && "fill-rose-500 text-rose-500")} />
              </button>
            </div>
            ...
            <div className="mt-4 flex items-center gap-4">
              ...
              {product.available ? (
                <span className="inline-flex items-center gap-1 text-xs text-emerald-600 font-medium"><Check className="h-3 w-3" /> Available now</span>
              ) : (
            ...
            <div className="mt-6 flex items-center gap-3 p-4 rounded-2xl border border-border">
              <img src={product.owner.avatar} alt={product.owner.name} className="h-11 w-11 rounded-full" />
              <div className="flex-1">
                <div className="text-sm font-medium">{product.owner.name}</div>
                <div className="text-xs text-muted-foreground">Owner · {product.owner.rating}★ · Responds in 1h</div>
              </div>
              <Button variant="outline" size="sm" onClick={() => navigate({ to: "/messages" })}>Message</Button>
            </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <Button size="lg" className="flex-1" leftIcon={<Calendar className="h-4 w-4" />} onClick={() => navigate({ to: "/checkout", search: { id: product.id } as never })} disabled={!product.available}>
                Rent now
              </Button>
